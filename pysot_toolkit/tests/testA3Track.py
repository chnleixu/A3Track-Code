...
from pysot_toolkit.trackers.net_wrappers import Backbone
...
def main(model_name, net_path):
    # create model
    net = Backbone(net_path=net_path, use_gpu=True)
    tracker = Tracker(name=model_name, net=net, window_penalty=0.49, exemplar_size=128, instance_size=256)
    ...
    for v_idx, video in enumerate(dataset):
        result_path = os.path.join('results', args.dataset, model_name, f'{video.name}.txt')
        ...
        for idx, (img, gt_bbox) in enumerate(video):
            print(f'({v_idx}) Video_name:{video.name}, Frame_id:{idx}')
...
if __name__ == '__main__': 
    model_name = 'A3Track'
    net_path = 'A3Track.pth.tar'
    main(model_name, net_path)
