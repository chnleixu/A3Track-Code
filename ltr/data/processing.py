...
class DeepProcessing(BaseProcessing):
    """ The processing class used for training TransT. The images are processed in the following way.
    First, the target bounding box is jittered by adding some noise. Next, a square region (called search region )
    centered at the jittered target center, and of area search_area_factor^2 times the area of the jittered box is
    cropped from the image. The reason for jittering the target box is to avoid learning the bias that the target is
    ...
    def __call__(self, data: TensorDict):
        ...
        # base_dir = '/home/richard_lx/debug'
        # num_file = len(os.listdir(base_dir))
        # output_dir = os.path.join(base_dir, 'test{}'.format(num_file+1))
        ...
        #     template_image = tensor_to_image(data['template_images'][i])
        #     cv2.imwrite(os.path.join(output_dir,'{}-template.jpg'.format(i)), template_image)
        
        # search_image = tensor_to_image(data['search_images'][0])
        # search_image = tensor_to_image(data['search_images'][0])
        # cv2.imwrite(os.path.join(output_dir,'search.jpg'), search_image)
